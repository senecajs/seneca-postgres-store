...
module.exports = function(opts) {
  ...
  function reconnect(args) {
    ...
    configure(opts, function(err) {
      if (err) {
        seneca.log.debug(
          'db reconnect (wait ' + opts.minwait + 'ms) failed: ' + err
        )
  ...
  var store = {
    ...
    save: function(args, done) {
      ...
      seneca.act(
        { role: actionRole, hook: 'save', target: name },
        args,
    ...
    load: function(args, done) {
      ...
      seneca.act(
        { role: actionRole, hook: 'load', target: name },
        args,
        function(err, queryObj) {
          var qent = args.qent
          var query = queryObj.query
    ...
    list: function(args, done) {
      ...
      seneca.act(
        { role: actionRole, hook: 'list', target: name },
        args,
        function(err, queryObj) {
          var qent = args.qent
          var q = args.q
    ...
    remove: function(args, done) {
      ...
      function executeRemove(args, done) {
        seneca.act(
          { role: actionRole, hook: 'remove', target: name },
          args,
          function(err, queryObj) {
            var query = queryObj.query
            if (err) {
          ...
          function(err, queryObj) {
            ...
            execQuery(query, function(err, res) {
              if (error(query, args, err, done)) {
                ...
                seneca.log.error(
                  'Query Failed',
                  JSON.stringify(errorDetails, null, 1)
                )
                return done(err)
              }
