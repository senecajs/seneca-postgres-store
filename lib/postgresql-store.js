...
module.exports = function (opts) {
  ...
  var store = {
      ...
      return getSchema(ent, sTypes, function (err, schema) {
        ...
        if (isUpdate) {
          return updateEnt(ent, sTypes, schema, opts, function (err, res) {
            ...
            if (!updatedAnything) {
              return insertEnt(ent, sTypes, done)
            }

            return findEnt(ent, { id: ent.id }, sTypes, done)
          })
        }
        ...
        return generateId(seneca, name, function (err, newId) {
          if (err) {
            return done(err)
          }
    ...
    load: function (args, done) {
      ...
      return findEnt(qent, q, sTypes, done)
    },
    ...
    list: function (args, done) {
      ...
      return listEnts(qent, q, sTypes, done)
    },
    ...
    remove: function (args, done) {
      ...
      return removeEnt(qent, q, sTypes, function (err, res) {
        ...
        var shouldLoad = !args.q.all$ && args.q.load$

        if (shouldLoad && res.rows.length > 0) {
          return done(null, makeEntOfRow(res.rows[0], args.ent))
        }
  ...
  function removeEnt(ent, q, sTypes, done) {
    ...
    return execQuery(delQuery, done)
  }
