...
module.exports = function (opts) {
  ...
  var store = {
    ...
    load: function (args, done) {
      ...
      var query = internals.selectstm(qent, q)
      var trace = new Error()
      execQuery(query, function (err, res) {
        if (error(query, args, err)) {
    ...
    list: function (args, done) {
      ...
      if (_.isString(q)) {
        ...
        if (q.distinct$ || q.fields$) {
          query = internals.filterStatement(qent, q)
        }
        else if (q.ids) {
          query = internals.selectstmOr(qent, q)
        }
        else {
          query = internals.selectstm(qent, q)
        }
      }
    ...
    remove: function (args, done) {
      ...
      if (q.all$) {
        ...
        var selectQuery = internals.selectstm(qent, q)
  ...
  internals.selectstm = function (qent, q) {
    var stm = {}
    ...
    if (!_.isEmpty(w) && w.params.length > 0) {
      w.params.forEach(function (param, i) {
        if (w.values[i] === null) {
          ...
          w.values.splice(i, 1)
          params.push('"' + internals.escapeStr(internals.camelToSnakeCase(param)) + '" IS NULL')
        }
        else if (w.values[i] instanceof RegExp) {
          var op = (w.values[i].ignoreCase) ? '~*' : '~'
          params.push('"' + internals.escapeStr(internals.camelToSnakeCase(param)) + '"' + op + '$' + (++cnt))
