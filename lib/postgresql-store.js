...
module.exports = function (opts) {
            ...
            return execQuery(query, function (err, res) {
              if (!error(query, args, err, done)) {
                ...
                if (should_load && entps.length > 0) {
                  ...
                  var newEnt = RelationalStore.makeent(args.ent, attrs)
  ...
  function execSave(args, schema, done) {
    return buildSaveStm(args, schema, function (err, queryObj) {
      ...
      var query = queryObj.query
      var saveOperation = queryObj.operation
      ...
      return execQuery(query, function (err, res) {
        ...
        if (saveOperation === 'update') {
          ...
          if (!updatedAnything) {
            var sTypes = specificTypes(args.target)
            var savestm = QueryBuilder.savestm(ent, sTypes)

            return execQuery(savestm, function (err, res) {
              if (err) {
                return done(err)
              }
              ...
              if (res.rows && res.rows.length > 0) {
                ...
                //
                var attrs = internals.transformDBRowToJSObject(res.rows[0])
                newEnt = RelationalStore.makeent(args.ent, attrs)
              }
          ...
          var loadArgs = { qent: ent, target: target, q: { id: ent.id } }

          return buildLoadStm(loadArgs, function (err, queryObj) {
            if (err) {
              return done(err)
            }
            ...
            return execQuery(query, function (err, res) {
              ...
              if (res.rows && res.rows.length > 0) {
                // NOTE: res.rows should always be an array of length === 1,
                // however we want to play it safe here, and not crash the client
                // if something goes awry (e.g. race condition).
                //
                var attrs = internals.transformDBRowToJSObject(res.rows[0])
                outEnt = RelationalStore.makeent(ent, attrs)
              }

              return done(null, outEnt)
            })
          })
        }
        ...
        seneca.log.debug({ tag$: args.tag$ }, saveOperation, newEnt)
  ...
  function buildSaveStm(args, schema, done) {
    ...
    if (update) {
      ...
      query = QueryBuilder.updatestm(ent, sTypes/*, schema, { merge }*/)
