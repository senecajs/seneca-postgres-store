...
module.exports = function (opts) {
  var seneca = this
  var snakeCamelParsing = {
    snakeToCamelCase: opts.snakeToCamelCase,
    camelCaseToSnake: opts.camelCaseToSnake
  }
  var StandardQuery = require('seneca-standard-query', snakeCamelParsing)
  seneca.use(StandardQuery)
  ...
  internals.transformDBRowToJSObject = function (row) {
    ...
    for (var attr in row) {
      if (row.hasOwnProperty(attr)) {
        obj[StandardQuery.snakeToCamelCase(attr)] = row[attr]
      }
    }
    return obj
  }
