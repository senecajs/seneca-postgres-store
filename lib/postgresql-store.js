...
module.exports = function (opts) {
  ...
  var store = {
    ...
    remove: function (args, done) {
      ...
      if (q.all$) {
        return executeRemove(args, done)
      }
      ...
      return buildLoadStm(Object.assign({}, args, { target: name }), function (err, queryObj) {
        ...
        if (err) {
          seneca.log.error('Error loading an entity', err)
          return done(err)
        }
        ...
        return execQuery(query, function (err, res) {
          ...
          if (entp) {
            // TODO: Tidy up.
            //
            var rm_args = _.clone(args)
            rm_args.q = _.clone(rm_args.q)
            rm_args.q.id = entp.id

            return executeRemove(rm_args, done)
          }
  ...
  function buildListStm(args, done) {
    ...
    var q = _.clone(args.q)
    if (null != q.limit$ && q.limit$ < 0) delete q.limit$
