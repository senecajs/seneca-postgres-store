...
module.exports = function (opts) {
  ...
  var store = {
    ...
    save: function (args, done) {
      ...
      return buildSchemaStm(Object.assign({}, args, { target: name }), function (err, schemaQueryObj) {
        if (err) {
          seneca.log.error('Postgres save error', err)

          var schemaQuery = schemaQueryObj ? schemaQueryObj.query : null
          var operation = schemaQueryObj ? schemaQueryObj.operation : null

          return done(err, {code: operation, tag: args.tag$, store: store.name, query: schemaQuery, error: err})
        }

        var schemaQuery = schemaQueryObj.query

        return execQuery(schemaQuery, function (err, res) {
          if (error(schemaQuery, args, err)) {
            seneca.log.error('Postgres save error', err)
            return done(err, {code: operation, tag: args.tag$, store: store.name, query: schemaQuery, error: err})
          }
          ...
          return buildSaveStm(Object.assign({}, args, { target: name }), schema, function (err, queryObj) {
            if (err) {
              ...
              var query = queryObj ? queryObj.query : null
              var operation = queryObj ? queryObj.operation : null
